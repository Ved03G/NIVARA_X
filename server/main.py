...
async def agent_context(req: AgentContextRequest) -> AgentResponse:
    ...
    has_images = len(req.canvas_images) > 0
    ...
    logger.info(
        ...
        f"{f' | canvas_images={len(req.canvas_images)}' if has_images else ''}"
    ...
    if len(req.elements) == 0 and not has_images and not has_ocr:
        return AgentResponse(
            actions=[],
            reasoning="No interactive elements or visual data found on page.",
            task_complete=False,
        )
